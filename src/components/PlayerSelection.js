...
const handleClick=(id)=>{
    ...
    const optionButtons= option.map((item)=>{
            return (
                    ...
                    <button onClick={()=>handleClick(item.id)} disabled={startGame?true:false} className='col-4 w-100  text-wrap btn btn-outline-success btn-sm text-left text-capitalize'>{item.selection}</button>
                  </div>) 
})
  ...
  return (
    <div>
      <div className='container w-100  p-2 m-2 mx-auto'>
        <div className="row">
          <div className="col ">
              <h3 className=" text-capitalize textColor text-center">{(startGame) && (playersChoice)}</h3>
          </div>
        </div>
          </div>
